...
export function LocationDetailSheet({
  ...
  const displayBranch = useSheetDisplayItem(branch, open);
        ...
        <div className="space-y-4 px-4 pb-4">
          <div className="inline-flex items-center gap-4 align-top">
            ...
            <div className="space-y-2">
              <p className="text-lg font-semibold">{displayBranch.name.trim() || "Location"}</p>
              <DetailSheetIconBadge icon={Power}>
                {displayBranch.isActive !== false ? "Active" : "Inactive"}
              </DetailSheetIconBadge>
            </div>
          </div>
          <LocationOverviewFields branch={displayBranch} onSaved={onSaved} />
        </div>
      </SheetContent>
    </Sheet>
  );
